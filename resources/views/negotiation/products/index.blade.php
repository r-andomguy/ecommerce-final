...
<x-app-layout>
    <div x-data="{ openDeleteModal: false, deleteItemId: null, successMessage: '' }" class="container mx-auto px-6 py-10">
        <div class="mb-4">
        ...
        <h1 class="text-4xl font-bold text-gray-900 text-center mb-6">
            Produtos da Negociação #{{ $negotiationId }}
        </h1>
        ...
        @if($negotiationProducts->count() === 0)
            <div class="text-center">
                <p class="text-gray-600 text-lg mb-4">Nenhum produto adicionado a esta negociação.</p>
            </div>
        @else
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-lg">
                    ...
                    <tr>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Produto</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Quantidade</th>
                    ...
                    @foreach($negotiationProducts as $product)
                        <tr data-product-id="{{ $product->id }}">
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $product->product }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $product->quantity }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700">R$ {{ number_format($product->total, 2, ',', '.') }}</td>
                            <td class="px-6 py-4 text-center">
                                <button
                                    @click="deleteItemId = {{ $product->id }}; openDeleteModal = true"
        ...
        <div x-show="openDeleteModal" @click.away="openDeleteModal = false" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                ...
                <div class="text-center mt-6 flex justify-center space-x-4">
                    ...
                    <button @click="deleteProduct({{$negotiationId}}, deleteItemId)"
    ...
    <script>
        function deleteProduct(negotiationId, productId) {
            console.log(`Excluindo produto ${productId} da negociação ${negotiationId}`);

            fetch(`/negotiation/${negotiationId}/products/${productId}`, {
                method: 'DELETE',
                headers: {
