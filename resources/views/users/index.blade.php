...
<x-app-layout>
    <div x-data="{ openCreateModal: false, openDeleteModal: false, deleteUserId: null, successMessage: '' }" class="container mx-auto px-6 py-10">
        <h1 class="text-4xl font-bold text-gray-900 text-center mb-6">Gerenciamento de Usuários</h1>

        <div class="text-end mb-6">
            <button @click="openCreateModal = true" class="py-2 px-4 bg-blue-500 text-white font-semibold rounded hover:bg-blue-600 flex items-center">
        ...
        @if($users->isEmpty())
            <div class="text-center">
                <p class="text-gray-600 text-lg mb-4">Nenhum usuário encontrado.</p>
            </div>
        @else
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-lg">
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Documento</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Tipo de Documento</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Telefone</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Nascimento</th>
                    ...
                    @foreach($users as $user)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $user->document_id }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $user->document_type }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700">{{ $user->phone_number }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700">{{ \Carbon\Carbon::parse($user->birth_date)->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center space-x-4">
                                    ...
                                    <button @click="deleteUserId = {{ $user->id }}; openDeleteModal = true" class="text-red-500 hover:text-red-700">
        ...
        <div x-show="openDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div class="bg-white p-6 rounded-lg shadow-lg w-96 relative">
                ...
                <p class="mt-4 text-gray-700">Tem certeza que deseja excluir o usuário <span x-text="deleteUserId" class="font-bold text-gray-900"></span>?</p>
                <div class="text-center mt-6 flex justify-center space-x-4">
                    <button @click="openDeleteModal = false" class="py-2 px-4 bg-gray-100 text-gray-700 border rounded hover:bg-gray-200">Cancelar</button>
                    <button @click="deleteUser(deleteUserId)" class="py-2 px-4 bg-red-500 text-white rounded hover:bg-red-600">Confirmar</button>
                </div>
